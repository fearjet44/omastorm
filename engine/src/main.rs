...
impl Shared {
    fn polar_site_id(&self) -> Option<&str> {
        match &self.state.selection {
            ...
            }) if source_id == "nexrad" => Some(site_id.as_str()),
            _ => None,
        }
    }
    ...
    fn select_source(&mut self, id: &str) -> (bool, Option<String>) {
        match self.registry.get(id).map(|a| a.family()) {
            Some(protocol::Family::Polar) => (
                false,
                Some(format!(
            ...
            Some(protocol::Family::Grid) if id == crate::grid_fixture::ID => {
                ...
                let frames = self.registry.fixture.frames();
                if frames.is_empty() {
                    return (false, Some("Fixture mosaic has no frames.".into()));
                }
                self.abort_live();
