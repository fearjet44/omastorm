...
use crate::{
    live,
    protocol::{
        AdapterTarget, Coverage, Family, GeoPoint, Kind, ProductClass, Selection, SourceInfo,
        Station,
    },
};
...
pub struct SourceMetadataBorrowed<'a> {
    ...
    pub coverage: AdapterCoverage<'a>,
}

#[allow(dead_code)]
pub enum AdapterCoverage<'a> {
    Sites(&'a [SiteCoverage]),
    Mosaic {
        coverage: &'a Coverage,
...
impl SourceRegistry {
    ...
    pub fn candidates(&self) -> Vec<Candidate> {
        let mut out = Vec::new();
        for site in &self.nexrad.sites {
            out.push(Candidate {
                selection: Selection {
                    source_id: self.nexrad.id.to_owned(),
        ...
        let meta = self.fixture.metadata();
...
impl Nexrad {
    ...
    pub fn metadata(&self) -> SourceMetadataBorrowed<'_> {
        // Sites() coverage is rebuilt each call for the trait shape; selection
        // uses `self.sites` directly so a pan does not allocate a polygon.
        SourceMetadataBorrowed {
            ...
            coverage: AdapterCoverage::Sites(&[]),
        }
    }
...
mod tests {
    ...
    fn fixture_box_is_far_from_nexrad() {
        ...
        let picked = registry.covering_selection(over_mosaic, None).unwrap();
        assert_eq!(picked.source_id, "fixture-mosaic");
